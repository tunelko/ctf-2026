...
128-byte buffer overflow into a 32-byte buffer. The binary implements a userland shadow stack that protects the return address of `play()`, but `do_reset()` (which calls `play()` recursively) has no protection. We preserve the `play()` return address to pass the verification check and overwrite the `do_reset()` return address with `win()`.
...
2. **Recursion + BOF** = ability to overwrite ancestor function frames on the call stack.
